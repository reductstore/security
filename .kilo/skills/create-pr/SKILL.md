---
name: create-pr
description: Create GitHub pull requests with the gh CLI using the repo-local PR template (.github/pull_request_template.md), then update CHANGELOG.md with the created PR ID and commit the changelog (do not push). Use when a user asks to open a PR and record its ID in the changelog.
---
...
Create a PR using the repo-local PR template, then record the PR number in CHANGELOG.md and commit the changelog without pushing.
...
### 3) Prepare the PR body from the local template
Use the repository PR template as the starting point:

```bash
cp .github/pull_request_template.md /tmp/pr_body.md
```

Fill in `/tmp/pr_body.md` with the relevant summary, scope, traceability, security impact, and verification
steps derived from the diff and conversation context. Remove sections that don't apply.
...
### scripts/
- `fetch_pr_template.sh`: Legacy helper (network) to download the reductstore/.github PR template.
